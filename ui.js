...
function updateTurnText() {
  ...
  yourTurnBanner.textContent = playerName === currentPlayer ? "🎯 ¡Es tu turno!" : "⏳ Esperando al oponente...";
}
...
function drawGrid() {
  ...
  if (lobby.playerHealth.Player1 <= 0 || lobby.playerHealth.Player2 <= 0) {
      document.body.innerHTML = `<h1 style='text-align:center; margin-top:100px;'>Game Over! ${
          lobby.playerHealth.Player1 <= 0 ? 'Player2' : 'Player1'
      } wins!</h1>`;
      return;
  }
  ...
  for (let i = 0; i < 100; i++) {
      ...
      if (playerName === currentPlayer && isAvailableMove(i) && !lobby.hasMoved) {
          cell.classList.add("highlight");
          cell.addEventListener("click", () => {
